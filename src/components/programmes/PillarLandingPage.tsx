...
export default function PillarLandingPage({ pillarKey, icon: PillarIcon, color, subcategories }: PillarLandingPageProps) {
  const t = useTranslations(`programmes.${pillarKey}`);
  const tCommon = useTranslations("common");

  const activeCourses = subcategories.flatMap((sub) => sub.courses.filter((c) => c.status === "active"));
  const futureCourses = subcategories.flatMap((sub) => sub.courses.filter((c) => c.status === "future"));
    ...
    <>
      ...
      <section className="pt-28 pb-16 bg-gradient-to-br from-[#0A1628] to-[#00438A]">
        <div className="container">
          <Link href="/programmes" className="inline-flex items-center gap-1 text-white/60 text-sm mb-6 hover:text-white/80 no-underline">
          ...
          <div className="flex gap-6 mt-8">
            <div className="text-center">
              <p className="text-2xl font-bold text-[#C4922A]">{activeCourses.length}</p>
              <p className="text-xs text-white/60">{tCommon("activeCourses")}</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-white/40">{futureCourses.length}</p>
        ...
        <div className="container">
          <div className="space-y-16">
            {subcategories.map((sub) => {
              ...
              return (
                <motion.div key={sub.id} id={sub.id} {...fadeInUp} className="scroll-mt-24">
                  <div className="flex items-center gap-3 mb-6">
                    <div className={`w-10 h-10 rounded-lg ${color} flex items-center justify-center`}>
                      <Icon className="w-5 h-5" />
                    </div>
                    <div>
                      <h2 className="text-xl font-bold text-[#0A1628]">{t(`subcategories.${sub.id}.title`)}</h2>
                      <p className="text-sm text-[#8A889A]">{t(`subcategories.${sub.id}.desc`)}</p>
                    </div>
                  </div>
          ...
          <div className="container">
            ...
            <div className="flex flex-wrap gap-2">
              {futureCourses.map((course) => (
                <span key={course.titleKey} className="px-3 py-1.5 bg-[#C4922A]/10 text-[#C4922A] text-xs font-medium rounded-full">
                  {t(`courses.${course.titleKey}`)}
                </span>
              ))}
            </div>
